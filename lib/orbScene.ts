...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  /** Feed a 0..1 amplitude level (e.g. from live agent audio) to drive core/bloom reactivity. */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  function animate() {
    ...
    // Smoothed live-voice reactivity (fed by setAudioLevel)
    audioLevel += (audioLevelTarget - audioLevel) * 0.3;
    const voiceSurge = audioLevel * audioLevel;
    ...
    const coreScale = 1 + surge + voiceSurge * 1.4 + Math.sin(t * 5) * 0.05;
    ...
    coreSphereMat.opacity = Math.min(0.9, coreOpacity + voiceSurge * 0.5);
    glowSphere.scale.setScalar(1 + surge * 0.8 + voiceSurge * 0.6);
    glowSphereMat.opacity = Math.max(0, (0.03 + surge * 0.08) * (1 - fadeOut * 0.9) + voiceSurge * 0.12);
    // Icosahedron wireframe stays visible even when glow fades
    icoWire.scale.setScalar(1 + surge * 0.6 + voiceSurge * 0.5);
    icoWireMat.opacity = Math.min(1, 0.5 + surge * 0.4 + voiceSurge * 0.5);
    ...
    // Bloom pulse
    bloom.strength = 1.6 + Math.sin(t * 0.8) * 0.3 + voiceSurge * 1.1;
